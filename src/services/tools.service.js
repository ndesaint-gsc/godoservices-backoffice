...
// GET /perfil/user/evbk/purchase?externalId=<id>
...
const findPurchaseByExternalId = async (externalId) => {
  const response = await fetch(apiUrl('/perfil/user/evbk/purchase' + toQueryString({ externalId })));
...
// POST /perfil/user/evbk/createUser  body: { email, brand }
const createUser = ({ email, brand }) =>
  http.post(apiUrl('/perfil/user/evbk/createUser'), { email, brand });

// POST /perfil/user/evbk/uploadNifs  multipart: { email, nifsFile }
...
const uploadNifs = async (csvFile, email) => {
  ...
  const response = await fetch(apiUrl('/perfil/user/evbk/uploadNifs'), {
    method: 'POST',
    body: formData,
  });
  if (!response.ok) {
